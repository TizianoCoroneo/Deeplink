...
// Now the center is ready to parse URLs!

let url = URL(string: "https://example.com/product/123abc")!
try! center.parse(url: url)

// This will run the closure that we registered for the `productDeeplink` template since the format matches, and it will print `ProductId: 123abc`.
